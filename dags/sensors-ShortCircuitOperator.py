...
# sustituir o check usando sensor por um ShortCircuitOperator
# se o sensor estiver lento, você pode usar esta opção, mas o ShortCircuitOperator não tem controle de tentativas, timeout,,,
...
# Função para verificar se a tabela existe no banco de dados SQLite
...
with DAG('Sensors-ShortCircuitOperator',
         default_args=default_args,
         description='Sersors - DAG ShortCircuitOperator',
    ...
    wait_for_file = FileSensor(
        ...
        fs_conn_id='sensors_filesystem',  # Conexão configurada no Airflow
        timeout=600,
        poke_interval=30,
        mode='poke'
    )
    ...
    extract_data_task = PythonOperator(
        ...
        python_callable=sil_extrai_dados
    )
    ...
    # ShortCircuitOperator para verificar se a tabela existe antes de prosseguir
    check_table_task = ShortCircuitOperator(
    ...
    start_task >> check_api_availability >> extract_data_task >> wait_for_file >> copy_task >> check_table_task >> insert_data_task >> end_task
